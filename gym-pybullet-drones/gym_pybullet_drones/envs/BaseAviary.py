...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.G = 9.8
        self.M = mass
        self.wVec = wind # Wind will first be modeled as constant field
        ...
        print("[INFO] BaseAviary.__init__() loaded parameters from the drone's .urdf:\n[INFO] M {:f} L {:f},\n[INFO] ixx {:f}, iyy {:f}, izz {:f},\n[INFO] kf {:f}, km {:f},\n[INFO] t2w {:f}, max_speed_kmh {:f},\n[INFO] gnd_eff_coeff {:f}, prop_radius {:f},\n[INFO] drag_xy_coeff {:f}, drag_z_coeff {:f},\n[INFO] dw_coeff_1 {:f}, dw_coeff_2 {:f}, dw_coeff_3 {:f}".format(
             self.origM, self.L, self.J[0,0], self.J[1,1], self.J[2,2], self.KF, self.KM, self.THRUST2WEIGHT_RATIO, self.MAX_SPEED_KMH, self.GND_EFF_COEFF, self.PROP_RADIUS, self.DRAG_COEFF[0], self.DRAG_COEFF[2], self.DW_COEFF_1, self.DW_COEFF_2, self.DW_COEFF_3))
        '''
        #### Compute constants #####################################
        ...
        '''
        for i in range(self.NUM_DRONES):
            print(p.getDynamicsInfo(self.DRONE_IDS[i], -1))
        '''
    ...
    def _housekeeping(self):
        ...
        #### EDIT: CUSTOM DYNAMICS FOR DOMAIN RANDOMIZATION #########
        if self.M != self.origM:
            for i in range(self.NUM_DRONES):
                p.changeDynamics(self.DRONE_IDS[i], -1, mass = self.M)
        self.wVec = np.zeros(3)
    ...
    def _downwash(self,
        ...
        """
        # Check for more than 1 drone in system
    ...
    def _dynamics(self,
        ...
        force_world_frame = thrust_world_frame - np.array([0, 0, self.GRAVITY]) + self.wVec
